...
async def uploadVideo(
    ...
    "Please stay comfy while we finish this~ (≧▽≦)\n"
    "🌸 Thanks so much for your patience! (*^_^*)",
    c_time,
)
            else:
                c_time = time.time()
        ...
        try:
            ...
            if upload_mode is False:
                ...
                sent_: Message = await c.send_video(
                    ...
                    progress_args = (
    f"✨ Uploading: `{merged_video_path.rsplit('/',1)[-1]}`\n"
    f"Please stay comfy while it completes~ (≧▽≦)\n"
    f"🌸 Thank you so much for your patience! (*^_^*)",
    c_time,
)
            else:
                c_time = time.time()
                sent_: Message = await c.send_document(
                    ...
                    progress_args = (
    f"✨ Uploading: `{merged_video_path.rsplit('/',1)[-1]}`\n"
    f"Please stay comfy while we finish~ (≧▽≦)\n"
    f"🌸 Thanks so much for your patience! (*^_^*)",
    c_time,
)
        ...
        if sent_ is not None:
            if Config.LOGCHANNEL is not None:
                ...
                await sent_.copy(
                    ...
                    caption = (
    f"✨ `{media.file_name}`\n\n"
    f"Merged successfully for <a href='tg://user?id={cb.from_user.id}'>{cb.from_user.first_name}</a>!\n"
    f"Please enjoy your video~ (≧▽≦)\n"
    f"🌸 Thanks so much for waiting! (*^_^*)"
)
...
async def uploadFiles(
    ...
    f"Please stay comfy while we finish~ (≧▽≦)\n"
    f"🌸 Thanks for your patience! (*^_^*)",
    c_time,
    f"\n**Uploading Progress:** {n}/{all} ✨",
),
)
        if sent_ is not None:
            if Config.LOGCHANNEL is not None:
...
caption = (
    f"✨ `{media.file_name}`\n\n"
    f"Successfully extracted for <a href='tg://user?id={cb.from_user.id}'>{cb.from_user.first_name}</a>!\n"
    f"Please enjoy your file~ (≧▽≦)\n"
    f"🌸 Thanks so much for waiting! (*^_^*)"
),
)